...
`;
    ...
    const hoverClass =
        hoverEffect === 'shadow' ? 'hover:shadow-[0_10px_15px_rgba(0,0,0,0.2)]' :
        hoverEffect === 'scale'  ? 'hover:scale-[1.05]' :
        hoverEffect === 'border' ? 'hover:border-zinc-900' : '';

    const generateTailwind = () => `<div class="w-[${width}px] bg-[${backgroundColor}] rounded-[${borderRadius}px] border-[${borderWidth}px] border-solid border-[${borderColor}] shadow-[${toTw(boxShadow)}] p-[${padding}px] overflow-hidden transition-all duration-[${Math.round(hoverTransitionDuration * 1000)}ms] ${hoverClass}">
  ${showImage ? `<img src="${imageUrl}" alt="Card image" class="w-full h-[${imageHeight}px] object-cover rounded-t-[${borderRadius}px] mb-[${padding}px]" />` : ''}
  ${showTitle ? `<h2 class="text-[${titleFontSize}px] font-[${titleFontWeight}] text-[${titleColor}] mb-[${padding / 2}px]">${titleText}</h2>` : ''}
  ${showDescription ? `<p class="text-[${descriptionFontSize}px] text-[${descriptionColor}] mb-[${padding}px]">${descriptionText}</p>` : ''}
  ${showButton ? `<button class="bg-[${buttonBackgroundColor}] text-[${buttonTextColor}] rounded-[${buttonBorderRadius}px] px-4 py-2 text-base cursor-pointer transition-colors hover:opacity-90">${buttonText}</button>` : ''}
</div>`;

    const generateReact = () => `import React from 'react';

export function CustomCard() {
  return (
    <div className="w-[${width}px] bg-[${backgroundColor}] rounded-[${borderRadius}px] border-[${borderWidth}px] border-solid border-[${borderColor}] shadow-[${toTw(boxShadow)}] p-[${padding}px] overflow-hidden transition-all duration-[${Math.round(hoverTransitionDuration * 1000)}ms] ${hoverClass}">
      ${showImage ? `<img src="${imageUrl}" alt="Card image" className="w-full h-[${imageHeight}px] object-cover rounded-t-[${borderRadius}px] mb-[${padding}px]" />` : ''}
      ${showTitle ? `<h2 className="text-[${titleFontSize}px] font-[${titleFontWeight}] text-[${titleColor}] mb-2">${titleText}</h2>` : ''}
      ${showDescription ? `<p className="text-[${descriptionFontSize}px] text-[${descriptionColor}] mb-4">${descriptionText}</p>` : ''}
